...
//            monga_main->connect((*config->mongo_main_host_.begin()), err);
        }
    }
...
void ParentDB::OfferLoad(mongo::Query q_correct)
{
    ...
    auto cursor = monga_main->query(config->mongo_main_db_ + ".offer", q_correct);
...
bool ParentDB::InformerLoadAll()
{
    if(!fConnectedToMainDatabase)
    {
        Log::warn("InformerLoadAll: not connected to data base");
        return false;
    }

    std::unique_ptr<mongo::DBClientCursor> cursor = monga_main->query(config->mongo_main_db_ + ".informer", mongo::Query());
    int skipped = 0;
    Kompex::SQLiteStatement *pStmt;
    int i = 0;
...
bool ParentDB::InformerUpdate(const std::string &id)
{
    ...
    std::unique_ptr<mongo::DBClientCursor> cursor = monga_main->query(config->mongo_main_db_ + ".informer", QUERY("guid" << id));
    Kompex::SQLiteStatement *pStmt;
...
void ParentDB::CategoriesLoad()
{
    ...
    auto cursor = monga_main->query(config->mongo_main_db_ + ".domain.categories", mongo::Query());
...
void ParentDB::CampaignLoad(const std::string &aCampaignId)
{
    ...
    {
        cursor = monga_main->query(config->mongo_main_db_ +".campaign", QUERY("guid" << aCampaignId));
    }
    else
    {
        cursor = monga_main->query(config->mongo_main_db_ +".campaign", mongo::Query());
    }
...
*/
        ...
        mongo::BSONObjIterator it = o.getObjectField("allowed").getObjectField("informers");
        while (it.more())
...
{
    auto cursor = monga_main->query(config->mongo_main_db_ +".campaign", QUERY("guid_int" << campaign_id));
